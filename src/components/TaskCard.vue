...
const excerpt = computed(() => {
  // %%comments%% are private notes; they never show on the board either.
  const noTitle = stripComments(props.task.body).replace(/^#\s+.+$/m, '').trim()
...
<template>
  ...
  >
    ...
    <div class="mt-2 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground">
      <span v-if="task.assignee" class="inline-flex items-center gap-1 bg-secondary px-2 py-0.5 font-medium">@{{ task.assignee }}</span>
      <span
        v-if="dueLabel"
        :class="['inline-flex items-center px-2 py-0.5 font-medium', tone === 'red' && 'bg-red-500/15 text-red-600', tone === 'amber' && 'bg-amber-500/15 text-amber-600', tone === 'muted' && 'bg-secondary text-secondary-foreground']"
      >
        {{ dueLabel }} · {{ task.dueDate }}
      </span>
      <span v-if="task.workspace" class="rounded bg-muted px-1.5 py-0.5 font-mono">{{ task.workspace }}</span>
      <span v-if="subtasks.length" :title="`${subtaskDone} of ${subtasks.length} subtasks done`" class="inline-flex items-center gap-1 bg-secondary px-2 py-0.5 font-medium">☑ {{ subtaskDone }}/{{ subtasks.length }}</span>
    </div>
    <div v-if="task.labels.length" class="mt-2 flex flex-wrap items-center gap-1">
      <span v-for="l in shownLabels" :key="l" class="border px-1.5 py-px text-[11px] text-muted-foreground">#{{ l }}</span>
