...
import { FolderOpen, RefreshCw, Unplug, LayoutGrid } from 'lucide-vue-next'
...
<template>
  <aside class="flex w-60 shrink-0 flex-col gap-3 overflow-y-auto border-r bg-card p-3">
    <UiButton @click="connectFolder()" size="sm" class="w-full">
      <FolderOpen class="h-4 w-4" /> Connect folder
    </UiButton>
    ...
    <button
      ...
      :class="['flex items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm', selectedProjectId === 'all' ? 'bg-accent font-medium' : 'hover:bg-accent/60']"
    >
      <LayoutGrid class="h-4 w-4" /> All projects
    </button>
    <div v-for="p in projects" :key="p.id" class="group">
      <button
        @click="selectProject(p.id)"
        :class="['flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm', selectedProjectId === p.id ? 'bg-accent font-medium' : 'hover:bg-accent/60']"
      >
      ...
      <div v-if="selectedProjectId === p.id" class="mt-1 flex gap-1 px-2">
        <button v-if="p.kind === 'fs'" @click="rescanProject(p.id)" class="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground" title="Rescan folder">
          <RefreshCw class="h-3 w-3" /> rescan
        </button>
    ...
    <div v-if="workspaces.length" class="mt-2">
      ...
      <button
        ...
        :class="['mt-1 flex w-full rounded-md px-2 py-1 text-left font-mono text-xs', selectedWorkspace === 'all' ? 'bg-accent font-semibold' : 'hover:bg-accent/60']"
      >
        all
      </button>
      <button
        ...
        :class="['flex w-full rounded-md px-2 py-1 text-left font-mono text-xs', selectedWorkspace === w ? 'bg-accent font-semibold' : 'hover:bg-accent/60']"
      >
        {{ w }}
      </button>
    </div>
    ...
    <div class="mt-auto flex flex-col gap-2 border-t pt-2">
      <UiButton variant="outline" size="sm" @click="rescanAll()" :disabled="loading">
        <RefreshCw class="h-3.5 w-3.5" /> Rescan all
      </UiButton>
